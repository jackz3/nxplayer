...
function Header() {
  ...
  let homeColor = pathname === '/' ? 'md:text-blue-700 md:bg-transparent text-white bg-blue-700' : 'md:text-gray-900'
  let aboutColor = pathname === '/about' ? 'text-white bg-blue-700 md:text-blue-700 md:bg-transparent' : 'text-gray-900'

  return (
    <nav className="bg-white border-gray-200 dark:bg-gray-900 sticky top-0 flex-shrink-0 border-b shadow-sm">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="mobile-menu">
          <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
            <li>
              <Link onClick={closeMenu} href="/" className={`block py-2 pl-3 pr-4 rounded ${homeColor} md:p-0 md:dark:text-blue-500`} aria-current="page">Home</Link>
            </li>
            <li>
              <Link onClick={closeMenu} href="/about" className={`block py-2 pl-3 pr-4 ${aboutColor} rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700`}>About</Link>
            </li>
          </ul>
        </div>
      </div>
